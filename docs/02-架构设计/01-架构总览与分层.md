...
当前单机方案的核心判断是「**长任务不占请求线程**」已做对（见 [04-高并发架构.md](../01-核心功能/04-高并发架构.md) 阶段 1 五项整改）；阶段 2/3 方向（PostgreSQL 写并发、Redis 缓存/分布式锁/审批状态、Agent 独立 worker 服务、JWT/SSO、K8s 化与消息队列）一句话指向 [高并发与部署架构.md](06-高并发与部署架构.md)，升级路径不欠债。
...
| [状态机设计.md](02-状态机设计.md) | 状态全集、转移表、持久化与并发安全 |
| [治理层设计.md](07-治理层设计.md) | 预算 / 守卫 / 工具白名单 / HITL / 幂等 |
| [RAG检索链路设计.md](04-RAG检索链路设计.md) | 混合检索选型与权衡（向量+FTS+RRF+rerank） |
| [高并发与部署架构.md](06-高并发与部署架构.md) | 单机并发模型与阶段 2/3 路线图 |
