...
| 向量索引（small-to-big 分块 / upsert / ANN 开关 / 维度校验） | `app/vector_kb.py` | `app/vector_kb.py:index_docs` / `app/vector_kb.py:upsert_docs` / `app/vector_kb.py:search` / `app/vector_kb.py:embed` / `app/vector_kb.py:_maybe_create_ann_index` | 🔌 `scripts/index_kb.py`（重建索引） | 02-RAG知识库.md |
