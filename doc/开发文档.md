...
> 面试讲稿见 `doc/面试文档.md`；开场一张图见 `README.md`。
...
- 但**企业治理层**（状态机、审批持久化、预算、Trace）如果交给框架的黑盒（如 LangGraph 的 checkpoint/interrupt），面试很难讲清「HITL 怎么中断、怎么恢复」。所以治理保留自研、数据全落 SQLite，每一行都能讲出「为什么这样设计」。
...
- 用户显式设 `MODEL_NAME` → 所有意图用它（OpenRouter 上可切任意模型 id，如 `openai/gpt-4o`）。
...
| Token `tokens` | 200k | 同上 |
| 时间 `seconds` | 600s | 同上 |

- SDK 路径：`Runner.run_sync(max_turns=budget.max_steps)` 限制轮数；每步工具调用 `budget.consume_step()`。
...
- **压**：单条 observation > `MAX_OBS_LEN=1200` 则头尾摘要。
...
| `query_user_dir` | 中 | Identity | 查用户/权限，返回脱敏 |
| `grant_db_readonly` | 高 | Change | 演示道具，HITL + 幂等 |
| `revoke_db_readonly` | 高 | Change | 补偿回收，HITL |
...
- `approvals`：HITL 审批（pending/approved/rejected）。
- `traces`：模型/工具/状态 Trace 全字段。
- `metrics`：每日指标（成功率/正确失败率/延迟/成本/人工接管率）。
- `memory`：长期记忆（当前仅示例）。
- `kb_fts`：知识库 FTS5 虚拟表（`app/kb.py`）。

启动时 `_migrate` 幂等补列（`budget_used_tokens` `budget_used_seconds` `prompt_version` `trace_req_id` `final_answer`）。
...
- 前端 `web/app/traces/[id]` 直接读 `traces` 表回放，**无需改动**。
...
- `GET /api/tickets/{id}/events`　SSE 事件流（EventSource 兼容，token 走 query）
...
MODEL_NAME=openrouter/free            # 或任意 OpenRouter 模型 id
...
- 冒烟脚本：
  - `scripts/smoke_agents.py`：真模型多 Agent handoff + HITL 恢复（需可访问 openrouter.ai）
...
- **审批 `RunState` 存进程内内存**（`_PENDING`）：服务重启后未完成审批需重新触发工单。若需跨重启，可改 `RunState.to_json()` 落库 + `from_json` 恢复（需 async）。
...
- **SSO**：当前 `api/auth` 为本地账户（`htpasswd` 风格），留有迁移点。
...
| HITL | `app/agents_runner.py` + `app/daos/approvals.py` + `app/api/tickets.py` |
| 预算 | `app/budget.py` |
| 上下文装配 | `app/context_assembler.py` |
| Trace / 指标 | `app/trace.py` `app/agents_trace.py` `app/metrics.py` |
