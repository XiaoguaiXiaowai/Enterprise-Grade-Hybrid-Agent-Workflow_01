...
# 规则兜底关键词：命中即归类（先意图后风险）。P2-15：扩充词表覆盖常见 IT 工单表达。
INTENT_RULES = [
    ("change", ("开通", "授权", "权限", "grant", "只读", "变更", "修改配置", "重置密码",
                "新建账号", "审批", "申请账号", "开账号", "注销账号", "禁用账号", "解封",
                "白名单", "加白", "回收", "提权", "降权", "sudo", "root", "改密", "初始化密码",
                "误删", "找回", "还原", "回滚", "恢复数据", "数据恢复")),
    ("troubleshoot", ("故障", "排查", "连不上", "上不去", "无法访问", "报错", "502", "宕机",
                      "卡顿", "不能联网", "不能上网", "ping", "断网", "掉线", "蓝屏", "死机",
                      "黑屏", "打不开", "闪退", "加载失败", "超时", "timeout", "504", "404",
                      "未响应", "没声音", "无画面", "连不上wifi", "vpn连不上")),
    ("data_query", ("数据", "查询", "导出", "报表", "统计", "dataset", "sql", "分析",
                    "拉数", "取数", "导数据", "数据核对", "查一下", "多少条", "明细")),
    ("knowledge", ("怎么", "如何", "流程", "文档", "指引", "请教", "咨询", "说明",
                   "步骤", "方法", "标准", "规范", "在哪", "地址", "入口", "费用", "制度")),
]
RISK_RULES = [
    ("high", ("上不去网", "宕机", "删除数据", "高危", "敏感", "生产故障", "全线", "崩溃",
              "删库", "清库", "删表", "drop", "truncate", "误删", "数据丢失", "数据泄露",
              "中毒", "勒索", "入侵", "攻破", "全公司", "全体", "核心系统", "主库")),
    ("medium", ("变更", "授权", "权限", "开通", "修改配置", "重启", "部署", "升级",
                "迁移", "扩容", "缩容", "加白", "回收", "重置", "新建账号", "注销账号")),
]
...
def relevance_prompt(title: str, description: str) -> str:
    ...
    return (
        ...
        "处理范围（如：知识咨询/IT故障排查说明、查询用户信息/权限/身份、"
        "开通/回收权限等高风险变更、查询服务器资产/网络设备/负责人等CMDB信息 等）。\n"
